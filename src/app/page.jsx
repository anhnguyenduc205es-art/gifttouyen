...
import { useVhVar } from "../utils/useVhVar"
import MailTruck from "./components/MailTruck"

export default function BirthdayApp() {
  const [currentScreen, setCurrentScreen] = useState(0)
  const [isLoading, setIsLoading] = useState(true)

  const birthdayDate = new Date("2025-08-14T20:58:00")
  const [isBirthdayOver, setisBirthdayOver] = useState(new Date().getTime() >= birthdayDate.getTime())

  // Sử dụng hook để hỗ trợ iOS cũ
  useVhVar()
  ...
  const screens = [
    !isBirthdayOver
      ...
      <MailTruck key="happy" onNext={() => setCurrentScreen(3)} />,
    <Letter key="letter" onNext={() => setCurrentScreen(4)} />,
    <PhotoGallery key="gallery"  />,
  ]

  return (
    <main className="flex-1 flex flex-col bg-gradient-to-br from-purple-950/30 via-black to-purple-950/30 overflow-hidden relative">
